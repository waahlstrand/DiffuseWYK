...
"""
DiffusionWYK model and criterion classes.
"""
import torch
...
class SetCriterionDynamicK(nn.Module):
    """This class computes the loss for DiffusionWYK.
    ...
    def __init__(
        ...
        if self.use_fed_loss:
            ...
            cls_weight_fun = lambda: get_fed_loss_cls_weights(
        ...
        if self.use_focal:
            self.focal_loss_alpha = cfg.MODEL.DiffusionWYK.ALPHA
            self.focal_loss_gamma = cfg.MODEL.DiffusionWYK.GAMMA
        else:
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=False):
        ...
        if self.use_focal or self.use_fed_loss:
            num_boxes = (
                torch.cat(target_classes_o_list).shape[0]
                if len(target_classes_o_list) != 0
                else 1
            )
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        for batch_idx in range(batch_size):
            ...
            pred_norm_box_list.append(
                bz_src_boxes[valid_query] / bz_image_whwh
        ...
        if len(pred_box_list) != 0:
            ...
            loss_bbox = F.l1_loss(
                src_boxes_norm, box_cxcywh_to_xyxy(target_boxes), reduction="none"
            )
            losses["loss_bbox"] = loss_bbox.sum() / num_boxes

            # loss_giou = giou_loss(box_ops.box_cxcywh_to_xyxy(src_boxes), box_ops.box_cxcywh_to_xyxy(target_boxes))
            loss_giou = 1 - torch.diag(
                box_ops.generalized_box_iou(src_boxes, target_boxes_abs_xyxy)
            )
...
class HungarianMatcherDynamicK(nn.Module):
    ...
    def __init__(
        ...
        self.use_focal = use_focal
        self.use_fed_loss = cfg.MODEL.DiffusionWYK.USE_FED_LOSS
        self.ota_k = cfg.MODEL.DiffusionWYK.OTA_K
        if self.use_focal:
            self.focal_loss_alpha = cfg.MODEL.DiffusionWYK.ALPHA
            self.focal_loss_gamma = cfg.MODEL.DiffusionWYK.GAMMA
        assert (
            ...
            for batch_idx in range(bs):
                ...
                if self.use_focal:
                    ...
                    pos_cost_class = (
                        alpha
                        * ((1 - bz_out_prob) ** gamma)
                        * (-(bz_out_prob + 1e-8).log())
                    )
                    cost_class = (
                        pos_cost_class[:, bz_tgt_ids] - neg_cost_class[:, bz_tgt_ids]
                    )
                ...
                bz_out_bbox_ = (
                    bz_boxes / bz_image_size_out
                )  # normalize (x1, y1, x2, y2)
                bz_tgt_bbox_ = (
                    bz_gtboxs_abs_xyxy / bz_image_size_tgt
                )  # normalize (x1, y1, x2, y2)
