...
@META_ARCH_REGISTRY.register()
class DiffusionWYK(DiffusionWYK):
    """
    ...
    def __init__(self, cfg):
        ...
        self.num_known = cfg.MODEL.DiffusionWYK.NUM_KNOWN  # Number of known labels
    ...
    def prepare_diffusion_concat(self, gt_boxes):
        ...
        num_known = min(self.num_known, num_gt)
        if num_known > 0:
            known_idx = torch.randperm(num_gt)[:num_known]

            # known_boxes = gt_boxes[known_idx]
        else:
            known_idx = torch.empty((0,), dtype=torch.long, device=self.device)
            unknown_idx = torch.arange(num_gt, dtype=torch.long, device=self.device)
        # known_boxes = torch.empty((0, 4), device=self.device)
